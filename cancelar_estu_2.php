<?php
    ...
    if (isset($_SESSION['usuario'])) {
        ...
        if (isset($_POST['cancelar_estu'])) {

            $turno             = $_POST['turno'];
            $fecha_registro    = $_POST['fecha'];

            $log = $log = "DELETE FROM `agenda` WHERE `id_estudiante` = '$nom' AND `fecha` = '$fecha_registro' AND `turno` = '$turno'";
            ...
            echo '<script> window.location="cancelar_estu.php"; </script>';
            ...
            $log = $log = "SELECT * FROM `agenda` WHERE `id_estudiante` = '$nom' AND `fecha` = '$fecha_registro' AND `turno` = '$turno'";
    ...
    include 'fecha.php';
        ...
        <div class="container">
            ...
            include 'conexion.php';
            $res_estu = mysqli_query($conectar, "SELECT * FROM `estudiantes` WHERE `id_alumno` = '$nom'");
            ...
            $res_agenda = mysqli_query($conectar, "SELECT * FROM `agenda` WHERE `id_estudiante` = '$nom' AND `fecha` = '$fecha_registro' AND `turno` = '$turno'");

            //se inicia ciclo while para imprimir datos en la tabla
            while ($row3 = mysqli_fetch_row($res_agenda))
            {
                $tutor = $row3[1];
                $turno = $row3[3];
                $motivo= $row3[4];
            ...
            //se inicia ciclo while para imprimir datos en la tabla
            while ($row4 = mysqli_fetch_row($res_turno)) {

                $hora_inicio = $row4[1];
                $hora_fin    = $row4[2];
            }

            $res_prof = mysqli_query($conectar, "SELECT * FROM `profesores` WHERE `id_prof` = '$tutor'");

            //se inicia ciclo while para imprimir datos en la tabla
            while ($row6 = mysqli_fetch_row($res_prof)) 
            {
                $ape_prof = $row6[1];
                $nom_prof = $row6[2];
            }
            ...
            echo '<div class="col-md-10 offset-md-1">
                        ...
                        <form action="cancelar_estu_2.php" method="post">
                        ...
                        <div class="form-row mt-4">
                            ...
                            <div class="col-sm-4 pb-3">
                                ...
                                <input type="text" class="form-control" value="' . $nom1 . '" disabled>
                            </div>
                            <div class="col-sm-4 pb-3">
                            ...
                            <div class="col-sm-4 pb-3">
                                <label for="exampleSt">Tutor asignado:</label>
                                <input type="text" class="form-control" value="' . $nom_prof . ' ' . $ape_prof . '" disabled>
                            </div>
                        ...
                        <center><button class="btn btn-lg btn-danger btn-block col-md-4" name="cancelar_estu" type="submit">CANCELAR TUTORÍA</button></center>
                        </form>
                    </div>';
            ?>
        ...
        <div class="container">
            <div class="row">
                <div id="bloque-presetancion" class="col-md-12">
                    ...
                    <hr/>
                    <center>
                        <form id="form-desbloqueo" action="cancelar_estu_2.php" method="post">
