<?php
    ...
    if (isset($_SESSION['usuario'])) {
        ...
        if ($tipo == 1) {
            ...
            if (isset($_POST['desbloqueoacceso2'])) {

                $codigo_desbloqueo = $_POST['codigo_desbloqueo'];

                $log = $log = "UPDATE `usuarios` SET `intentos`= '0' WHERE `id_usuario` = '$codigo_desbloqueo'";

                $sintac = mysqli_query($conectar, $log) or die("Algo ha ido mal en la consulta a la base de datos");

                echo '<script> alert("Se ha desbloqueado el Acceso");</script>';
                echo '<script> window.location="desbloqueoacceso.php"; </script>';

            } else if (isset($_POST['desbloqueoacceso'])) {

                $codigo_desbloqueo = $_POST['codigo_desbloqueo'];

                $log = $log = "SELECT * FROM `usuarios` WHERE `id_usuario` = '$codigo_desbloqueo'";
                ...
                if (mysqli_num_rows($sintac) == 0) {
                    ...
                    echo '<script> alert("Este usuario no tiene el acceso bloqueado o no esta registrado.");</script>';
                    echo '<script> window.location="desbloqueoacceso.php"; </script>';

                } else {
                    //sentencias de consultas
                    $resfull = mysqli_query($conectar, "SELECT * FROM `usuarios` WHERE `id_usuario` = '$codigo_desbloqueo'");
                    ...
                    if ($bloqueado == '1') {
                        ...
                        echo '<script> alert("Esta cuenta ha sido suspendida.");</script>';
                        echo '<script> window.location="desbloqueoacceso.php"; </script>';

                    } else if ($intentos <= '2') {

                        echo '<script> alert("Este usuario no tiene el acceso bloqueado.");</script>';
                        echo '<script> window.location="desbloqueoacceso.php"; </script>';

                    } else if ($tipo == '1') {

                        $res_estu = mysqli_query($conectar, "SELECT * FROM `estudiantes` WHERE `id_alumno` = '$codigo_desbloqueo'");
                        ...
                        $res_prof = mysqli_query($conectar, "SELECT * FROM `profesores` WHERE `id_prof` = '$codigo_desbloqueo'");
    ...
    include 'fecha.php';
        ...
        <div class="container">
            <div class="row">
              ...
              <td>' . $numero_acu . '</td>
              <td><center>
                 <form class="text-right" id="boton-formulario" action="desbloqueoacceso2.php" method="post">
                 <input type="hidden" name="codigo_desbloqueo" class="form-control" value="' . $codigo_desbloqueo . '">
                  <button class="btn btn-success" name="desbloqueoacceso2" type="submit">DESBLOQUEAR ACCESO</button>
                 </form>
                 </center>
              </tr>';
        ...
        <div class="container">
            <div class="row">
                <div id="bloque-presetancion" class="col-md-12">
                    <br>
                    <p class="text-center">
                        <strong>Desbloqueo de Acceso<br>
            </strong>
                    </p>
                    <hr/>
                    <center>
                        <form id="form-desbloqueo" action="desbloqueoacceso2.php" method="post">
                            <div class="text-center mb-4">
                                <img class="mb-2" src="images/bloquear.png" alt="" width="auto" height="80">
                                <hr>
                                <h1 class="h3 mb-3 font-weight-normal access"><b>DESBLOQUEO DE ACCESO</b></h1>
                            </div>
                            <div class="form-label-group">
                                <!--<label for="inputEmail">ID de usuario</label> -->
                                <input type="text" name="codigo_desbloqueo" class="form-control" placeholder="ID de usuario" value="<?php echo " $codigo_desbloqueo "; ?>" required>
                            </div>
                            <br>
                            <button class="btn btn-lg btn-success btn-block" name="login" type="submit">BUSCAR</button>
                        </form>
                    </center>
                    <br>
                </div>
